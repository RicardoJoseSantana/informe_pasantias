...
\section{Software utilizado.}
    ...
    \begin{itemize}
        \item \textbf{PlatformIO en VSCode:} Entorno de desarrollo integrado (IDE) basado en Visual Studio Code, que facilita la programación y gestión de proyectos para microcontroladores, incluyendo el ESP32. Proporciona una amplia gama de bibliotecas y herramientas para la compilación, carga y depuración del código.
        
        \begin{figure}[H]
            \centering
            \includegraphics[height=5cm]{04-figuras/platformio-interfaz.png}
            \caption{Interfaz de PlatformIO en VSCode.}
            \label{fig:platformio-interfaz}
        \end{figure}

        \begin{figure}[H]
            \centering
            \includegraphics[height=5cm]{04-figuras/platformio-config.png}
            \caption{Configuración de PlatformIO en VSCode.}
            \label{fig:platformio-config}
        \end{figure}

        \item \textbf{Librerías de Espressif:} Se emplearon diversas librerías específicas para facilitar la comunicación con el hardware, como la librería para manejar los pulsos del encoder rotatorio PCNT y la librería LiquidCrystal\_I2C para controlar la pantalla LCD a través del adaptador I2C.
        \item \text{Git y Github:} Utilizados para el control de versiones del código fuente y la colaboración en el desarrollo del proyecto \cite{Santana2025GitHub}.
        \item \textbf{Software de diseño PCB (KiCad):} Empleado para diseñar el esquema eléctrico y la placa de circuito impreso (PCB) del módulo basado en ESP32.
        \item \textbf{Google AI Studio:} Inteligencia artificial utilizada para sugerencias y correcciones de código durante el desarrollo del firmware. 
    \end{itemize}
...
\label{sec:driver-motor}
    ...
    \subsection{Caracterización del driver}
        ...
        Por otra parte también se indican las señales necesarias para establecer el control (fig \ref{fig:base-driver-senales})
    ...
    \subsection{Cálculo de pulsos por revolución y configuración de microstepping}
        ...
        \begin{itemize}
            \item $f_{pulsos}$: frecuancia de los pulsos enviados por el terminal PUL
            \item $f_{motor}$: frecuencia de revoluciones del motor (RPM/60)
            \item $PPR$: pulsos por revolución del motor (dependiendo del engranaje y microstepping)
        \end{itemize}
    ...
    \begin{itemize}
        \item Verificar que la corriente de ajuste del driver sea la adecuada (evitar sobrecalentamiento).
        \item Observar vibraciones a microsteppings bajos y calor del motor a altas frecuencias.
        \item Si se experimenta pérdida de pasos, reducir la velocidad, aumentar corriente o mejorar la aceleración (rampado) de la secuencia de pulsos.
    \end{itemize}
...
\section{fabricación de módulo de trabajo (basado en esp32)}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[height=5cm]{04-figuras/kicad-esquematico.png}
        \caption{Diagrama esquemático del módulo de trabajo.}
        \label{fig:kicad-esquematico}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[height=5cm]{04-figuras/kicad-placa.jpg}
        \caption{Layout del módulo de trabajo.}
        \label{fig:kicad-placa}
    \end{figure}